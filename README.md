...
| <a name="input_finding_severity_labels"></a> [finding\_severity\_labels](#input\_finding\_severity\_labels) | A list of finding severity labels used to filter which Security Hub findings<br/>are forwarded to notifications, e.g. `["CRITICAL", "HIGH"]`.<br/><br/>When empty (the default), findings of all severities matching<br/>`cloudwatch_event_rule_pattern_detail_type` are forwarded, preserving prior<br/>behavior. When set, only findings whose `Severity.Label` is in the list are<br/>forwarded.<br/><br/>For the list of valid severity labels, see:<br/>https://docs.aws.amazon.com/securityhub/1.0/APIReference/API_Severity.html | `list(string)` | `[]` | no |
